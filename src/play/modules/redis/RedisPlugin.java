...
public class RedisPlugin extends PlayPlugin {
    ...
    private boolean isFlushAtApplicationExit = false;

    public boolean isFlushAtApplicationExit() {
        return isFlushAtApplicationExit;
    }
    ...
    public void onApplicationStart() {
        if(Play.configuration.containsKey("redis.url")) {
            ...
            RedisConnectionInfo redisConnInfo = new RedisConnectionInfo(redisUrl,
                    Play.configuration.getProperty("redis.timeout"));
            ...
            while(Play.configuration.containsKey("redis." + nb + ".url")) {
                RedisConnectionInfo redisConnInfo = new RedisConnectionInfo(
                        Play.configuration.getProperty("redis." + nb + ".url"),
                        Play.configuration.getProperty("redis.timeout"));
                shards.add(redisConnInfo.getShardInfo());
                nb++;
            }

            RedisConnectionManager.shardedConnectionPool = new ShardedJedisPool(new JedisPoolConfig(), shards,
        ...
        RedisConnectionInfo(String redisUrl, String timeoutStr) {
            ...
            try {
                ...
                throw new ConfigurationException(
                        "Bad configuration for redis: unable to parse redis url (" + redisUrl + ")");
            }
